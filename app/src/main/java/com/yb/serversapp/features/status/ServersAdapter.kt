...
class ServersAdapter: ListAdapter<ServerViewItem, RecyclerView.ViewHolder>(
    AsyncDifferConfig.Builder(DIFF_CALLBACK).build()
) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType) {
            SERVER_TYPE_VIEWTYPE -> ServerTypeViewHolder.create(parent)
            SERVER_STATUS_VIEWTYPE -> ServerStatusViewHolder.create(parent)
    ...
    class ServerStatusViewHolder(private val binding: ItemServerBinding): RecyclerView.ViewHolder(binding.root) {
        ...
        companion object {
            fun create(parent: ViewGroup): ServerStatusViewHolder {
                return ServerStatusViewHolder(ItemServerBinding.inflate(LayoutInflater.from(parent.context), parent, false))
            }
        }
    }
